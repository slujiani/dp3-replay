...
class real_robot_eval:
    ...
    def eval(self):
        ...
        for i in range(iterations):
            ...
            # pcs_tensor=collect_current_cropped_point_cloud()  #获取裁剪好的 处理好的点云
            # print("********点云********")
            # point_cloud=pcs_tensor.unsqueeze(0)
            # obs['point_cloud']=point_clouds.unsqueeze(0) 
            ...
            for action in action_list:
                    self.head_pc.move_relative(action)  # 将动作发送到机械臂
                    pcs_tensor=collect_current_cropped_point_cloud()  #获取裁剪好的 处理好的点云
                    point_clouds.append(pcs_tensor)
                    
                    agent_pos=self.head_pc.get_current_pose()  #获取机械臂的观测值，和点云的帧数相同
                    agent_pos_tensor=torch.Tensor(agent_pos)
                    agent_poses.append(agent_pos_tensor)
            point_clouds = torch.stack(point_clouds, dim=0)
            agent_poses = torch.stack(agent_poses, dim=0)
            obs['point_cloud']=point_clouds.unsqueeze(0) 
            obs['agent_pos']=agent_poses.unsqueeze(0)
            with torch.no_grad():
