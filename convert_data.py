...
def fps_point_cloud(frame_data, x_range=(-0.2, 0.11), y_range=(-0.5, 0.27), z_range=(0.73, 1.15), num_points=1024):
    # 提取 x, y, z 坐标
    ...
    if len(filtered_points) <= num_points:
        ...
        # 对应的 RGB 信息
        # 由于 downsampled_pcd 只包含点信息，我们需要按照原始点云的索引映射来获取 RGB 数据
        # selected_rgb = filtered_rgb[:len(selected_points), :]
        # selected_rgb = filtered_rgb[selected_points]
