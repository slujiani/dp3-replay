...
import time
import torch
from convert_data import preprocess_point_cloud

def collect_current_cropped_point_cloud(frames_num) -> torch.Tensor:
    print("++++++++++++++++++")
    ...
    try:
        while frame_ind<frames_num:
            ...
            prossessed_pc_frame=preprocess_point_cloud(point_cloud_frame)
            point_cloud_tensor = torch.from_numpy(prossessed_pc_frame) #numpy array转换成torch.tensor
            print(f"get the {frame_ind}  cropped point_cloud_frame")
            pc_frames.append(point_cloud_tensor)
            frame_ind+=1
    finally:            
            pipeline.stop()
            return torch.stack(pc_frames, dim=0)
